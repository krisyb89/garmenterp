      ...
      {!loading && lineItems.length > 0 && (
        <div className="overflow-x-auto rounded-xl border border-gray-200 shadow-sm">
          <table className="min-w-full text-xs border-collapse">
            ...
            <thead>
              <tr>
                <th className="sticky left-0 z-20 bg-white border-b border-r border-gray-200 px-3 py-2 text-left text-gray-400 font-medium" rowSpan={2}>
                  PO #
                </th>
                <th className="sticky left-[80px] z-20 bg-white border-b border-r border-gray-200 px-3 py-2 text-left text-gray-400 font-medium" rowSpan={2}>
                  Style
                </th>
                <th className="bg-white border-b border-r border-gray-200 px-3 py-2 text-left text-gray-400 font-medium" rowSpan={2}>
                  Color / Print
                </th>
...
function CellModal({ cell, lineItem, onClose }) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await fetch(`/api/wip-cells/${cell.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status }),
      });
      // If there's a comment, we create a WIPComment directly
      if (comment.trim()) {
        await fetch('/api/wip-cells/' + cell.id + '/comments', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ text: comment, approvalStatus: status }),
        }).catch(() => {}); // optional endpoint — comment appended if route exists
