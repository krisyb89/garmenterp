...
export default function CostingEditPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    fetch('/api/costing-sheets/by-id/' + id)
      .then(r => r.json())
      .then(d => {
        setCosting(d);
      })
  ...
  async function saveCosting() {
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/costing-sheets/by-id/' + id, {
  ...
  if (!costing) return <div className="text-center py-20 text-red-500">Costing sheet not found</div>;
  ...
  const styleNo = cs.style?.styleNo || cs.srs?.styleNo || '—';
  const customerName = cs.style?.customer?.name || cs.srs?.customer?.name || '—';
  const sourceLabel = cs.srsId ? 'SRS' : 'Direct';
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold">{styleNo}</h1>
          <p className="text-gray-500 text-sm">{customerName} · Costing Sheet · {sourceLabel}</p>
        </div>
